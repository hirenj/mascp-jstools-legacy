...
jQuery(sequenceRenderer.getAminoAcidsByPosition(this)).each(function(){this.addToLayer('phosphat_peptide_'+i,{'height':20,'offset':-2.5});icons=icons.concat(this.addToLayer('phosphat_experimental',{'height':20,'offset':-2.5}));});});jQuery(MASCP.getGroup('phosphat_peptides')).bind('visibilityChange',function(e,rend,vis){if(rend!=sequenceRenderer){return;}
...
jQuery(renderer).trigger('resultsRendered',[reader]);});};(function(){var apply_map=function(data_block){var map=this.map;var databits=data_block.data;var headers=databits.shift();var dataset={};var id_col=headers.indexOf(map.id);var cols_to_add=[];for(var col in map){if(col=="id"){continue;}
