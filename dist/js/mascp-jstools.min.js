...
request.onreadystatechange=function(evt){if(request.readyState==4){if(request.status==200){callback.call(null,null,parsedata(JSON.parse(request.responseText)));}else{callback.call(null,{'cause':{'status':request.status}});}}};request.send();});};var basic_get_document=get_document;get_document=function(doc,etag,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');get_document_using_script(doc_id,function(err,dat){if(err){console.log("Retrying with authentication");basic_get_document(doc_id,etag,callback);}else{callback.call(null,null,dat);}});};}
...
console.log("Wiping out data");MASCP.Service.ClearCache(reader);reader.map=map;reader.setData(doc,data);});}
