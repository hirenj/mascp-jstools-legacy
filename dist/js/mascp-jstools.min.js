...
var old=reader.gotResult;reader.gotResult=function(){var index=0;for(var i=0;i<self.sequences.length;i++){if(self.sequences[i].agi&&self.sequences[i].agi==reader.agi){index=i;}}
...
for(var i=0;i<aligned.length;i++){var layname=self.sequences[i].agi.toLowerCase()||"missing"+i;var lay=MASCP.registerLayer(layname,{'fullname':self.sequences[i].name||layname.toUpperCase(),'group':group_name,'color':'#ff0000'});lay.fullname=self.sequences[i].name||layname.toUpperCase();var text_array=renderer.renderTextTrack(layname,aligned[i].toString());text_array[0].setAttribute('dy','-1ex');rendered_bits=rendered_bits.concat(text_array);rendered_bits.slice(-1)[0].layer=layname;if(renderer.trackOrder.indexOf(layname.toLowerCase())<0){renderer.trackOrder.push(layname.toLowerCase());}
