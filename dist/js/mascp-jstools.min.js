...
return-1*seq.length;};})();MASCP.ClustalRunner.prototype.setupSequenceRenderer=function(renderer){var self=this;jQuery(renderer).bind('readerRegistered',function(ev,reader){if(self==reader){return;}
var old=reader.gotResult;reader.gotResult=function(){var index=0;for(var i=0;i<self.sequences.length;i++){if(self.sequences[i].agi&&self.sequences[i].agi==reader.agi){console.log(reader.agi);index=i;}}
var old_get_aas=renderer.getAminoAcidsByPosition;var old_get_pep=renderer.getAminoAcidsByPeptide;renderer.getAminoAcidsByPosition=function(aas){var new_aas=aas.map(function(aa){return Math.abs(self.result.calculatePositionForSequence(index,aa));});return old_get_aas.call(this,new_aas);};renderer.getAminoAcidsByPeptide=function(){};old.call(reader);renderer.getAminoAcidsByPosition=old_get_aas;renderer.getAminoAcidsByPeptide=old_get_pep;}});var rendered_bits=[];var controller_name='isoform_controller';var group_name='isoforms';var redraw_alignments=function(sequence_index){var result=self.result;while(rendered_bits.length>0){var bit=rendered_bits.shift();renderer.remove(bit.layer,bit);}
result.alignToSequence(sequence_index||0);var aligned=result.getSequences();if(!renderer.sequence){renderer.setSequence(aligned[sequence_index])(function(){MASCP.registerGroup(group_name,'Splice variants');MASCP.registerLayer(controller_name,{'fullname':'Splices','color':'#000000'});if(renderer.trackOrder.indexOf(controller_name)<0){renderer.trackOrder.push(controller_name);}
...
for(var i=0;i<aligned.length;i++){MASCP.registerLayer(aligned[i].agi,{'fullname':aligned[i].agi,'group':group_name,'color':'#ff0000'});var text_array=renderer.renderTextTrack(aligned[i].agi,aligned[i].toString());rendered_bits=rendered_bits.concat(text_array);rendered_bits.slice(-1)[0].layer=aligned[i].agi;if(renderer.trackOrder.indexOf(aligned[i].agi)<0){renderer.trackOrder.push(aligned[i].agi);}
var name="Isoform "+(i+1);if(aligned[i].insertions){for(var insert in aligned[i].insertions){var insertions=aligned[i].insertions;var layname=aligned[i].agi;if(insert==0&&insertions[insert]==""){continue;}
if(insert==0){insert=1;}
var an_anno=renderer.getAA(insert).addToLayer(layname,{'content':'+'+insertions[insert].length,'height':16,'offset':1,'angle':320,'border':'rgb(0,0,255)','no_tracer':true})[1];rendered_bits.push(an_anno);rendered_bits.slice(-1)[0].layer=layname;}}}};this.bind('resultReceived',function(){var result=this.result;redraw_alignments(0);renderer.bind('orderChanged',function(e,order){var re=/[^\d]/g;redraw_alignments(parseInt(order[1].replace(re,'')));});});}
