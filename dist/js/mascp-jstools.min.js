...
var query=encodeURIComponent("title='"+prefs_domain+"' and mimeType = 'application/json+domaintool-session' and trashed = false");do_request("www.googleapis.com","/drive/v2/files?q="+query,null,function(err,data){if(err){callback.call(null,err);return;}
...
var uri=parseUri(data.downloadUrl);do_request(uri.host,uri.relative,null,function(err,data){console.log("Back from getting data");if(err){callback.call(null,err);return;}
...
var query=encodeURIComponent("title='"+prefs_domain+"' and mimeType = 'application/json+domaintool-session' and trashed = false");do_request("www.googleapis.com","/drive/v2/files?q="+query,null,function(err,data){if(err){callback.call(null,err);return;}
var item_id=null;if(data.items&&data.items.length==0){do_request("www.googleapis.com","/drive/v2/files/",null,arguments.callee,"POST:application/json",JSON.stringify({"title":prefs_domain,"mimeType":"application/json+domaintool-session","description":"Domaintool session information for session "+prefs_domain}));return;}
if(data.items){item_id=data.items[0].id;}else{item_id=data.id;}
var req=gapi.client.request({'path':"/upload/drive/v2/files/"+item_id,'method':"PUT",'params':{"uploadType":"media"},'headers':{'Content-Type':'application/json+domaintool-session'},'body':JSON.stringify(MASCP.preferences[prefs_domain])});req.execute(function(err,data){if(err){callback.call(null,err);return;}
...
head.appendChild(script);};var initing_auth=false;var waiting_callbacks=[];authenticate=function(cback,noevent){if(MASCP.GOOGLE_AUTH_TOKEN){cback.call(null);return;}
...
self.getDocument(doc,etag,function(e,data){if(e){if(e.cause.status==304){console.log("Matching e-tag");bean.fire(reader,'ready');return;}
...
if(update_timestamps[doc]&&((new Date().getTime())-update_timestamps[doc])<1000*60*120){bean.fire(reader,'ready');return;}
