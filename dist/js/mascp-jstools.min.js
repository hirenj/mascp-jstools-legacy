...
a_text.replaceChild(document.createTextNode(seq.substr(start,max_length)),a_text.firstChild);a_text.setAttribute('dx',5+((start)*RS));if(MASCP.IE){a_text.setAttribute('textLength',parseInt(5+((start)*RS))+(max_length*RS));}};if(!container.panevents){canvas.addEventListener('panstart',function(){if(amino_acids_shown){amino_acids.attr({'display':'none'});}},false);bean.add(canvas,'panend',function(){if(amino_acids_shown){amino_acids.attr({'display':'block'});update_sequence();}});container.panevents=true;}
