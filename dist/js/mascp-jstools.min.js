...
var nl="\n";bean.fire(self,'running');return{type:"GET",dataType:"txt",data:{"query":'<?xml version="1.0" encoding="UTF-8"?>'+'<!DOCTYPE Query>'+'<Query  virtualSchemaName = "default" formatter = "CSV" header = "0" uniqueRows = "0" count = "" datasetConfigVersion = "0.6" >'+'<Dataset name = "pride" interface = "default" ><Filter name = "submitted_accession_option" value = "'+self.agi+'"/>'+'<Attribute name = "peptide_sequence" /><Attribute name = "start_coord" /><Attribute name = "end_coord" />'+'</Dataset>'+'</Query>'}}};MASCP.PrideRunner.prototype.setupSequenceRenderer=function(renderer){this.bind('resultReceived',function(){var raw_values=[];var max_val=0;this.result._raw_data.data.forEach(function(pep){if(pep.peptide.length<1){return;}
