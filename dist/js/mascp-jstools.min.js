...
bean.add(scrollContainer,'scroll',function(){if(disabled||!console){return;}
bean.fire(controlElement,'panstart');var width=scroller.cached_width||scroller.clientWidth;target.setLeftPosition(parseInt(scrollContainer.scrollLeft*target.getTotalLength()/width));bean.fire(controlElement,'panend');});bean.add(controlElement,'pan',function(){var size=100*target.getTotalLength()/(target.getVisibleLength());scroller.style.width=parseInt(size)+'%';var width=scroller.cached_width||scroller.clientWidth;var left_shift=parseInt(width*(target.getLeftPosition()/target.getTotalLength()));disabled=true;scrollContainer.scrollLeft=left_shift;setTimeout(function(){disabled=false;},0);});};GOMap.Diagram.addScrollZoomControls=function(target,controlElement,precision){precision=precision||0.5;var self=this;var hookEvent=function(element,eventName,callback){if(typeof(element)=='string'){element=document.getElementById(element);}
