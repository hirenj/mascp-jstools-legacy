...
script.src="https://spreadsheets.google.com/feeds/cells/"+doc_id+"/1/"+type+"/basic?alt=json-in-script&callback=cback"+doc_id+""+auth;var error_function=function(){if(window.removeEventListener){window.removeEventListener(error_function);}
...
try{head.appendChild(script);}catch(e){callback.call(null,{"cause":{"status":""},"object":e});}};var initing_auth=false;var waiting_callbacks=[];authenticate=function(cback,noevent){if(MASCP.GOOGLE_AUTH_TOKEN){cback.call(null);return;}
...
setTimeout(function(){gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;window.setTimeout(function(){console.log("Google token has timed out, forcing refresh");delete MASCP["GOOGLE_AUTH_TOKEN"];},parseInt(result.expires_in)*1000);initing_auth=false;cback.call(null);waiting_callbacks.forEach(function(cb){if(cb!==cback){cb.call(null);}});waiting_callbacks=[];return;}else if(result&&result.error){initing_auth=false;var error={"cause":result.error};cback.call(null,error);waiting_callbacks.forEach(function(cb){if(cb!==cback){cb.call(null,error);}});waiting_callbacks=[];}else{initing_auth=false;if(auth_settings.immediate){if(!user_action){var auth_func=function(success){auth_settings.immediate=false;gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;window.setTimeout(function(){console.log("Google token has timed out, forcing refresh");delete MASCP["GOOGLE_AUTH_TOKEN"];},parseInt(result.expires_in)*1000);success.call(null);}else{success.call(null,{"cause":result?result.error:"No auth result"});}});};cback.call(null,{"cause":"No user event","authorize":auth_func});if(waiting_callbacks){waiting_callbacks.forEach(function(cb){if(cb!==cback){cb.call(null,{"cause":"No user event","authorize":auth_func});}});waiting_callbacks=[];}
return;}
