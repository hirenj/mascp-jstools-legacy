...
idb=true;var req=indexedDB.open("datacache");req.onsuccess=function(e){idb=e.target.result;var version="1.3";if(idb.version!=version){var set_version=idb.setVersion(version);set_version.onsuccess=function(e){if(idb.objectStoreNames.contains("cached")){idb.deleteObjectStore("cached");}
var store=idb.createObjectStore("cached",{keyPath:"id"});store.createIndex("entries",["acc","service"],{unique:false});store.createIndex("timestamps",["service","retrieved"],{unique:false});store.createIndex("services","service",{unique:false});var trans=set_version.result;trans.oncomplete=function(){if(MASCP.events){MASCP.events.emit("ready");}
if(MASCP.ready){MASCP.ready();}else{MASCP.ready=true;}};};}else{if(MASCP.events){MASCP.events.emit("ready");}
if(MASCP.ready){MASCP.ready();}else{MASCP.ready=true;}}};}else{try{db=openDatabase("cached","","MASCP Gator cache",1024*1024);}catch(err){throw err;}
