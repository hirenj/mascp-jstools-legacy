...
var auth_settings={client_id:MASCP.GOOGLE_CLIENT_ID,scope:scope,immediate:true};gapi.auth.authorize({immediate:true},function(){});initing_auth=true;gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;window.setTimeout(function(){console.log("Google token has timed out, forcing refresh");delete MASCP["GOOGLE_AUTH_TOKEN"];},parseInt(result.expires_in)*1000);initing_auth=false;cback.call(null);waiting_callbacks.forEach(function(cb){cb.call(null);});waiting_callbacks=[];return;}else if(result&&result.error){initing_auth=false;var error={"cause":result.error};cback.call(null,error);waiting_callbacks.forEach(function(cb){cb.call(null,error);});}else{if(auth_settings.immediate){auth_settings.immediate=false;gapi.auth.authorize(auth_settings,arguments.callee);return;}}});return;};do_request=function(host,path,etag,callback,method,data){authenticate(function(err){if(err){callback.call(null,err);return;}
...
request.onreadystatechange=function(evt){if(request.readyState==4){if(request.status<300){var datablock=request.responseText.length>0?(request.getResponseHeader('Content-Type').match(/json/)?JSON.parse(request.responseText):request.responseText):null;callback.call(null,null,datablock);}else{callback.call(null,{'cause':{'status':request.status}});}}};request.send(data);});};var basic_get_document=get_document;get_document=function(doc,etag,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');if(etag||MASCP.GOOGLE_AUTH_TOKEN){basic_get_document(doc,etag,callback);}else{get_document_using_script(doc_id,function(err,dat){if(err){basic_get_document(doc,etag,callback);}else{callback.call(null,null,dat);}});}};}
