...
try{head.appendChild(script);}catch(e){callback.call(null,{"cause":{"status":""},"object":e});}};var initing_auth=false;var waiting_callbacks=[];var has_failed_once=false;authenticate=function(cback,noevent){if(MASCP.GOOGLE_AUTH_TOKEN){cback.call(null);return;}
...
var request=new XMLHttpRequest();var req_method=method||'GET';request.open(req_method.replace(/:.*/,''),"https://"+host+path);request.setRequestHeader('Authorization','Bearer '+MASCP.GOOGLE_AUTH_TOKEN);if(req_method=='GET'){request.setRequestHeader('GData-Version','3.0');}
...
get_document_using_script(doc_id,callback);}else{callback.call(null,null,dat);}});}else{get_document_using_script(doc_id,function(err,dat){if(err){basic_get_document(doc,etag,function(err,dat){if(err){if(err.cause=="No user event"){callback.call(null,err);return;}
