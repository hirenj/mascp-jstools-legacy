...
if(targ.addListener){targ.addListener(ev,cback);}else{var callback_func=function(){var args=Array.prototype.slice.call(arguments);if(args[0]===targ){args.shift();cback.call(targ,args);}};MASCP.events.addListener(ev,callback_func);targ._listeners=targ._listeners||{};targ._listeners[cback]=callback_func;}},'remove':function(targ,ev,cback){var self=this;if(ev=="error"){ev="MASCP.error";}
if(cback&&targ.removeListener){targ.removeListener(ev,cback);}else if(cback){if(self._listeners&&self._listeners[cback]){MASCP.events.removeListener(ev,self._listeners[cback]);delete self._listeners[cback];}}else if(targ.removeAllListeners&&typeof cback=='undefined'){targ.removeAllListeners(ev);}},'fire':function(targ,ev,args){if(ev=="error"){ev="MASCP.error";}
if(targ.emit){targ.emit.apply(targ,[ev].concat(args));}else{MASCP.events.emit.apply(MASCP.events,[ev,targ].concat(args));}}};MASCP.events=new events.EventEmitter();module.exports=MASCP;var parser=require('jsdom').jsdom;var XMLHttpRequest=require("xmlhttprequest").XMLHttpRequest;Object.defineProperty(XMLHttpRequest.prototype,"responseXML",{get:function(){return parser((this.responseText||'').replace(/&/g,'&amp;'));},set:function(){}});XMLHttpRequest.prototype.customUA='MASCP Gator crawler (+http://gator.masc-proteomics.org/)';}else{window.MASCP=MASCP;var ie=(function(){var undef,v=3,div=document.createElement('div'),all=div.getElementsByTagName('i');do{div.innerHTML='<!--[if gt IE '+(++v)+']><i></i><![endif]-->';}while(all[0]);return v>4?v:undef;}());if(ie){if(ie===7){MASCP.IE=true;MASCP.IE7=true;}
