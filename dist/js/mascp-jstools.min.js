...
if(JSandbox){var sandbox=new JSandbox();var parser;sandbox.exec('var sandboxed_parser = '+pref.parser_function+';',function(){var box=this;parser=function(datablock,cback){box.eval({"data":"sandboxed_parser(datablock)","input":{"datablock":datablock},"callback":function(r){cback.call(null,r);}});};parser.callback=true;});}else{console.log("No sandbox support - not trying to get data for "+pref.title);callback.call(null,{"error":"No sandbox support"});return;}
var a_reader=(new MASCP.GoogledataReader()).createReader(set,parser);a_reader.bind('ready',function(){callback.call(null,null,pref,a_reader);});a_reader.bind('error',function(err){callback.call(null,{"error":err},pref);});};MASCP.IterateServicesFromConfig=function(configuration,callback){if(!configuration){return;}
