...
var new_nodes=new_owner._importNode(doc,true);if(typeof XPathResult!=='undefined'){var iterator=new_owner.evaluate('//svg:defs/*',new_nodes,function(ns){return svgns;},XPathResult.ANY_TYPE);var el=iterator.iterateNext();var to_append=[];while(el){to_append.push(el);el=iterator.iterateNext();}
...
objects.forEach(function(object){var click_reveal;var rendered;if(object.type==="box"){if(object.aa){rendered=renderer.getAA(parseInt(object.aa)).addBoxOverlay(track,parseInt(object.width),1,object.options);}else if(object.peptide){rendered=renderer.getAminoAcidsByPeptide(object.peptide).addToLayer(track,object.options);}}
