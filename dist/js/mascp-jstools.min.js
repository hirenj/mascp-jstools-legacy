...
retdata.retrieved=new Date(data.feed.updated.$t);retdata.retrieved.setUTCHours=function(){};retdata.retrieved.setUTCMinutes=function(){};retdata.retrieved.setUTCSeconds=function(){};retdata.retrieved.setUTCMilliseconds=function(){};retdata.etag=data.feed.gd$etag;retdata.title=data.feed.title.$t;return retdata;};var get_document,get_document_list,get_permissions,get_permissions_id,authenticate,do_request;get_document_list=function(callback){do_request("docs.google.com","/feeds/default/private/full/-/spreadsheet?alt=json",null,function(err,data){var results=[];if(data){var entries=data.feed.entry;var i;for(i=entries.length-1;i>=0;i--){results.push([entries[i].title.$t,entries[i]['gd$resourceId'].$t,new Date(entries[i]['updated'].$t)]);}}
...
do_request=function(host,path,etag,callback){var headers_block={'GData-Version':'3.0'};if(MASCP.GOOGLE_AUTH_TOKEN){headers_block['Authorization']='Bearer '+MASCP.GOOGLE_AUTH_TOKEN;}else{var self_func=arguments.callee;authenticate(function(){self_func.call(null,host,path,etag,callback);});return;}
if(etag){headers_block["If-None-Match"]=etag;}
require('https').get({host:host,path:path,headers:headers_block,},function(res){res.setEncoding('utf8');var response="";res.on('data',function(chunk){response+=chunk;});res.on('end',function(){if(res.statusCode!=200){callback.call(null,{'cause':{'status':res.statusCode}});return;}
var data=JSON.parse(response);callback.call(null,null,data);});});};MASCP.GoogledataReader.authenticate=authenticate;}else{var get_document_using_script=function(doc_id,callback){var head=document.getElementsByTagName('head')[0];var script=document.createElement('script');var type="public";var auth="";script.type='text/javascript';script.setAttribute('id','ssheet-'+doc_id);if(MASCP.GOOGLE_AUTH_TOKEN){auth="&access_token="+MASCP.GOOGLE_AUTH_TOKEN;type="private";}
