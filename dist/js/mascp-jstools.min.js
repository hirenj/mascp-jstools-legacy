...
get_permissions=function(doc,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
...
request.onreadystatechange=function(evt){if(request.readyState==4){if(request.status<300&&request.status>=200){var datablock=request.responseText.length>0?(request.getResponseHeader('Content-Type').match(/json/)?JSON.parse(request.responseText):request.responseText):null;callback.call(null,null,datablock);}else{callback.call(null,{'cause':{'status':request.status}});}}};request.send(data);});};var basic_get_document=get_document;get_document=function(doc,etag,callback){if(!doc&&callback){authenticate(callback,true);return;}
