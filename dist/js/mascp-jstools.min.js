...
return 1-(positives/(seqs.length-1));};var redraw_alignments=function(sequence_index){var result=self.result;while(rendered_bits.length>0){var bit=rendered_bits.shift();renderer.remove(bit.layer,bit);}
...
var alignments=result.getAlignment().split('');rendered_bits=rendered_bits.concat(renderer.renderTextTrack(controller_name,result.getAlignment().replace(/ /g,'.')));rendered_bits.slice(-1)[0].layer=controller_name;for(var i=0;i<alignments.length;i++){rendered_bits.push(renderer.getAA(i+1).addBoxOverlay(controller_name,1,check_values(aligned[0],i,aligned)));rendered_bits.slice(-1)[0].layer=controller_name;}
