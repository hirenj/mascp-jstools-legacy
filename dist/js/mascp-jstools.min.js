...
{bean.fire(this,'resultReceived');};MASCP.HydropathyRunner.prototype.setupSequenceRenderer=function(renderer){this.bind('resultReceived',function(){var windowSize=5;var options={};var kd={'A':1.8,'R':-4.5,'N':-3.5,'D':-3.5,'C':2.5,'Q':-3.5,'E':-3.5,'G':-0.4,'H':-3.2,'I':4.5,'L':3.8,'K':-3.9,'M':1.9,'F':2.8,'P':-1.6,'S':-0.8,'T':-0.7,'W':-0.9,'Y':-1.3,'V':4.2};var values=[];for(var i=0;i<windowSize;i++){values[i]=0;}
...
values.push(value);}
MASCP.registerLayer('hydropathy',{'fullname':'Hydropathy plot','color':'#f00'});renderer.addValuesToLayer('hydropathy',values,options);if(renderer.trackOrder.indexOf('hydropathy')<0){renderer.trackOrder.push('hydropathy');}
renderer.showLayer('hydropathy');renderer.trigger('resultsRendered',[this]);});};if(typeof MASCP=='undefined'||typeof MASCP.Service=='undefined'){throw"MASCP.Service is not defined, required class";}
...
var nl="\n";bean.fire(self,'running');return{type:"GET",dataType:"txt",data:{"query":encodeURIComponent('<?xml version="1.0" encoding="UTF-8"?>'+'<!DOCTYPE Query>'+'<Query  virtualSchemaName = "default" formatter = "CSV" header = "0" uniqueRows = "0" count = "" datasetConfigVersion = "0.6" >'+'<Dataset name = "pride" interface = "default" ><Filter name = "submitted_accession_option" value = "'+self.agi+'"/>'+'<Attribute name = "peptide_sequence" /><Attribute name = "start_coord" /><Attribute name = "end_coord" />'+'</Dataset>'+'</Query>')}}};MASCP.PrideRunner.prototype.setupSequenceRenderer=function(renderer){this.bind('resultReceived',function(){var raw_values=[];var max_val=0;this.result._raw_data.data.forEach(function(pep){if(pep.peptide.length<1){return;}
...
var plot=renderer.addValuesToLayer(this.agi,values,{'height':12,'offset':32,'label':{'max':max_val+' PRIDE peptides'}});plot.setAttribute('stroke','#00f');renderer.trigger('resultsRendered',[this]);});};(function(serv){var defaultDataReceived=serv.prototype._dataReceived;serv.prototype._dataReceived=function(data,status)
{if(data===null){return defaultDataReceived.call(this,null,status);}
if(typeof data=="object"){if(data.status&&data.status=="RUNNING"){var self=this;bean.fire(self,"running");setTimeout(function(){self.retrieve(self.agi);},5000);console.log("Got back running status");return;}}
...
plot.setHeight=function(height){var path_vals=recalculate_plot(0.5*height/RS);plot.setAttribute('d','M0 0 m0 '+height*offset_scale+'m0 '+0.5*height*height_scale+' '+path_vals);plot.setAttribute('stroke-width',RS/renderer.zoom);};axis.setHeight=function(height){if(abs_min_val<0&&abs_max_val>0){axis.setAttribute('d','M0 0 m0 '+height*offset_scale+'m0 '+0.5*height*height_scale+' l'+renderer._sequence_els.length*RS+' 0');}else{axis.setAttribute('d','M0 0 m0 '+height*offset_scale+'m0 '+0.5*(1-abs_min_val)*height*height_scale+' l'+renderer._sequence_els.length*RS+' 0');}
