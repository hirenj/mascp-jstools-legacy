...
callback.call(null,null,results);});});};get_document=function(doc,etag,callback){var headers_block={'GData-Version':'3.0'};var feed_type='public';if(MASCP.GOOGLE_AUTH_TOKEN){headers_block['Authorization']='Bearer '+MASCP.GOOGLE_AUTH_TOKEN;feed_type='private';}
if(etag){headers_block["If-None-Match"]=etag;}
var req=require('https').get({host:"spreadsheets.google.com",path:"/feeds/cells/"+doc+"/1/"+feed_type+"/basic?alt=json",headers:headers_block,},function(res){res.setEncoding('utf8');var data="";res.on('data',function(chunk){data+=chunk;});res.on('end',function(){var response=JSON.parse(data);callback.call(null,null,parsedata(response));});});};}else{get_document_list=function(callback){authenticate();var feedUrl="https://docs.google.com/feeds/default/private/full/-/spreadsheet";var service=new google.gdata.client.GoogleService('writely','gator');service.getFeed(feedUrl,function(data){var results=[];if(data){var entries=data.feed.entry;var i;for(i=entries.length-1;i>=0;i--){results.push([entries[i].title.$t,entries[i]['gd$resourceId'].$t,new Date(entries[i]['updated'].$t)]);}}
