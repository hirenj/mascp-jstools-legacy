...
var content=draw_discontinuity(renderer._canvas);content.setAttribute('fill','#ffff00');var an_anno=renderer.getAA(insert).addToLayer(layname,{'content':content,'bare_element':true,'height':20,'offset':-2.5,'no_tracer':true})[1];an_anno.container.setAttribute('height','300');an_anno.container.setAttribute('viewBox','-50 -100 200 300');rendered_bits.push(an_anno);rendered_bits.slice(-1)[0].layer=layname;}}}};this.bind('resultReceived',function(){var result=this.result;redraw_alignments(0);var accs=[];self.sequences.forEach(function(seq){accs.push(seq.agi);});renderer.bind('orderChanged',function(e,order){redraw_alignments(accs.indexOf(order[(order.indexOf(controller_name)+1)]));});});}
