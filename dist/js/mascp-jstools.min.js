...
if(!opts){opts={};}
var height=this._renderer._layer_containers[layerName].trackHeight||4;var text=canvas.text(this._index,0,opts.txt||"Text");text.setAttribute('font-size',0.75*height*this._renderer._RS);text.setAttribute('font-weight','bolder');text.setAttribute('fill','#ffffff');text.setAttribute('stroke','#000000');text.setAttribute('stroke-width','5');text.setAttribute('style','font-family: Helvetica, Arial, Gil Sans, sans-serif; text-anchor: middle;');text.firstChild.setAttribute('dy','2ex');text.setAttribute('text-anchor','middle');text.setHeight=function(height){text.setAttribute('font-size',0.75*height);};this._renderer._layer_containers[layerName].push(text);return text;}
...
var layer_containers=this._layer_containers||[];if(layer_containers[layer.name]){layer_containers[layer.name].forEach(function(el){if(el.parentNode){el.parentNode.removeChild(el);}});this.removeAnnotations(layer);this._layer_containers[layer.name]=null;layer.disabled=true;}};clazz.prototype.enablePrintResizing=function(){var self=this;if(self._media_func){return;}
...
self.zoomCenter=null;self._container_canvas.removeAttribute('viewBox');self.withoutRefresh(function(){self.zoom=zoomFactor;});self.grow_container=true;self.refresh();self.grow_container=false;};(self.win()||window).matchMedia('print').addListener(self._media_func);};clazz.prototype.wireframe=function(){var order=this.trackOrder||[];var y_val=0;var track_heights=0;if(!this.wireframes){return;}
