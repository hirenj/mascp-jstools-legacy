...
if(JSandbox){var sandbox=new JSandbox();var parser;sandbox.exec('var sandboxed_parser = '+pref.parser_function+';',function(){var box=this;parser=function(datablock,cback){box.eval({"data":"sandboxed_parser(input.datablock)","input":{"datablock":datablock},"callback":function(r){cback.call(null,r);console.log("Terminating");box.terminate();},"onerror":function(){console.log(arguments);}});};parser.callback=true;parser.terminate=function(){if(sandbox){sandbox.terminate();}};var a_reader=(new MASCP.GoogledataReader()).createReader(set,parser);a_reader.bind('ready',function(){if(parser){parser.terminate();}
...
var defs_block=this._container_canvas.lastChild;if(!new_owner._importNode){add_import(new_owner);}
var new_nodes=new_owner._importNode(doc,true);if(typeof XPathResult!=='undefined'){var iterator=new_owner.evaluate('//svg:defs/*',new_nodes,function(ns){console.log(ns);return svgns;},XPathResult.ANY_TYPE);var el=iterator.iterateNext();var to_append=[];while(el){to_append.push(el);el=iterator.iterateNext();}
