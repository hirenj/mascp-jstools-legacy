...
get_document_list=function(callback){var headers_block={'GData-Version':'3.0'};if(MASCP.GOOGLE_AUTH_TOKEN){headers_block['Authorization']='Bearer '+MASCP.GOOGLE_AUTH_TOKEN;}else{var self_func=arguments.callee;authenticate(function(){self_func.call(null,callback);});return;}
var req=require('https').get({host:"docs.google.com",path:"/feeds/default/private/full/-/spreadsheet?alt=json",headers:headers_block,},function(res){res.setEncoding('utf8');var response="";res.on('data',function(chunk){response+=chunk;});res.on('end',function(){var data=JSON.parse(response);var results=[];if(data){var entries=data.feed.entry;var i;for(i=entries.length-1;i>=0;i--){results.push([entries[i].title.$t,entries[i]['gd$resourceId'].$t,new Date(entries[i]['updated'].$t)]);}}
callback.call(null,null,results);});});};get_document=function(doc,etag,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');var headers_block={'GData-Version':'3.0'};var feed_type='public';if(MASCP.GOOGLE_AUTH_TOKEN){headers_block['Authorization']='Bearer '+MASCP.GOOGLE_AUTH_TOKEN;feed_type='private';}else{var self_func=arguments.callee;authenticate(function(){self_func.call(null,doc,etag,callback);});return;}
if(MASCP.GOOGLE_AUTH_TOKEN){headers_block['Authorization']='Bearer '+MASCP.GOOGLE_AUTH_TOKEN;feed_type='private';}
if(etag){headers_block["If-None-Match"]=etag;}
var req=require('https').get({host:"spreadsheets.google.com",path:"/feeds/cells/"+doc_id+"/1/"+feed_type+"/basic?alt=json",headers:headers_block,},function(res){res.setEncoding('utf8');if(res.statusCode!=200){callback.call(null,{'cause':{'status':res.statusCode}});return;}
var data="";res.on('data',function(chunk){data+=chunk;});res.on('end',function(){var response=JSON.parse(data);callback.call(null,null,parsedata(response));});});};MASCP.GoogledataReader.authenticate=authenticate;}else{var get_document_using_script=function(doc_id,callback){var head=document.getElementsByTagName('head')[0];var script=document.createElement('script');var type="public";var auth="";script.type='text/javascript';script.setAttribute('id','ssheet-'+doc_id);if(MASCP.GOOGLE_AUTH_TOKEN){auth="&access_token="+MASCP.GOOGLE_AUTH_TOKEN;type="private";}
...
var auth_settings={client_id:MASCP.GOOGLE_CLIENT_ID,scope:scope,immediate:true};gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;cback();}else{if(auth_settings.immediate){auth_settings.immediate=false;gapi.auth.authorize(auth_settings,arguments.callee);}}});return;};get_document_list=function(callback){authenticate(function(err){if(err){callback.call(null,err);return;}
var feedUrl="https://docs.google.com/feeds/default/private/full/-/spreadsheet";var service=new google.gdata.client.GoogleService('writely','gator');service.getFeed(feedUrl,function(data){var results=[];if(data){var entries=data.feed.entry;var i;for(i=entries.length-1;i>=0;i--){results.push([entries[i].title.$t,entries[i]['gd$resourceId'].$t,new Date(entries[i]['updated'].$t)]);}}
callback.call(null,null,results);},callback);});};get_document=function(doc,etag,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');get_document_using_script(doc_id,function(err,dat){if(err){console.log("Retrying with authentication");authenticate(function(err){if(err){callback.call(null,err);return;}
var request=new XMLHttpRequest();request.open("GET","https://spreadsheets.google.com/feeds/cells/"+doc_id+"/1/"+"private"+"/basic?alt=json");request.setRequestHeader('Authorization','Bearer '+MASCP.GOOGLE_AUTH_TOKEN);request.setRequestHeader('GData-Version','3.0');if(etag){request.setRequestHeader('If-None-Match',etag);}
request.onreadystatechange=function(evt){if(request.readyState==4){if(request.status==200){callback.call(null,null,parsedata(JSON.parse(request.responseText)));}else{callback.call(null,{'cause':{'status':request.status}});}}};request.send();});}else{callback.call(null,null,dat);}});};}
var render_site=function(renderer){var self=this;var sites=self.result._raw_data.sites||[],i=0,match=null;MASCP.registerLayer(self.datasetname,{'fullname':self.result._raw_data.title});for(i=sites.length-1;i>=0;i--){if(match=sites[i].match(/(\d+)/g)){renderer.getAminoAcidsByPosition([parseInt(match[0])])[0].addToLayer(self.datasetname);}}};var render_peptides=function(renderer){var self=this;var peptides=self.result._raw_data.peptides||[],i=0,match=null;MASCP.registerLayer(self.datasetname,{'fullname':self.result._raw_data.title});for(i=peptides.length-1;i>=0;i--){if(match=peptides[i].match(/(\d+)/g)){renderer.getAminoAcidsByPosition(parseInt(match[0])).addToLayer(self.datasetname);}}};var setup=function(renderer){this.bind('resultReceived',function(e){render_peptides.call(this,renderer);render_site.call(this,renderer);});};MASCP.GoogledataReader.prototype.getDocumentList=get_document_list;MASCP.GoogledataReader.prototype.getDocument=get_document;MASCP.GoogledataReader.prototype.createReader=function(doc,map){var self=this;var reader=new MASCP.UserdataReader();reader.datasetname=doc;reader.setupSequenceRenderer=setup;MASCP.Service.CacheService(reader);(function(){var a_temp_reader=new MASCP.UserdataReader();a_temp_reader.datasetname=doc;MASCP.Service.CacheService(a_temp_reader);MASCP.Service.CachedAgis(a_temp_reader,function(accs){if(accs.length<1){get_data(null);return;}
