...
marker.setAttribute('transform','translate('+((cx)*RS)+','+0.5*cy*RS+') scale(1)');marker.setHeight=setHeight;marker.setAttribute('height',dim.R*RS);if(typeof symbol=='string'){if(symbol.match(/^(:?https?:)?\//)){marker.contentElement=this.use(symbol,-r,0,r,r);}else{marker.contentElement=this.text_circle(0,0.5*r,1.75*r,symbol,opts);}
...
if(symb.match(/^(:?https?:)?\//)){new_el=canvas.use(symb,(x_pos-0.5)*r,(y_pos-0.5)*r,r,r);new_el.setAttribute('pointer-events','none');}else{new_el=canvas.text_circle(x_pos*r,y_pos*r,1.75*r,symb,opts);}
...
var dim={CX:cx+units,CY:cy+units,R:r+units,MIN_X:(cx-r)+units,MAX_X:(cx+r)+units,MIN_Y:(cy-r)+units,MAX_Y:(cy+r)+units,MID_X1:(cx-(r/2))+units,MID_X2:(cx+(r/2))+units,MID_Y1:(cy-(r/2))+units,MID_Y2:(cy+(r/2))+units};var marker_group=this.group();var text=this.text(0,dim.CY-0.5*dim.R,txt);text.setAttribute('font-size',(opts.font_size||r)*RS);text.setAttribute('font-weight',opts.weight||'bolder');text.setAttribute('fill',opts.text_fill||'#ffffff');text.setAttribute('style','font-family: sans-serif; text-anchor: middle;');text.firstChild.setAttribute('dy','1.5ex');text.setAttribute('text-anchor','middle');var back;if(!opts.stretch){back=this.circle(0,dim.CY,9/10*dim.R);}else{var text_width=3/2*text.getBBox().width/RS;var text_height=3/2*dim.R;var left_pos=-0.5*text_width;if(text_width>(3*dim.R)){left_pos=-0.5*text_width;if(opts.stretch=='right'){left_pos=-0.1*text_width;}
if(opts.stretch=='left'){left_pos=-0.9*text_width;}}else{text_width=3*dim.R;left_pos=-0.5*text_width;}
text.setAttribute('x',(0.5*text_width+left_pos)*RS);back=this.roundRect(left_pos,dim.CY-0.5*text_height,text_width,text_height,{'x':0.5*dim.R,'y':0.5*text_height},opts);}
...
if(object.type=="marker"){var content=(object.options||{}).content;var wanted_height=object.options.height;if(Array.isArray&&Array.isArray(content)){delete object.options.height;click_reveal=renderer.getAA(parseInt(object.aa)).addToLayer(track,object.options);object.options.height=wanted_height;click_reveal=click_reveal[1];click_reveal.style.display='none';object.options.content=object.options.alt_content;}else if(typeof(content)=='object'){var content_el;if(content.type=="circle"){content_el=renderer._canvas.circle(-0.5,-0.5,1,1);}
...
var added=renderer.getAA(parseInt(object.aa)).addToLayer(track,object.options);if(click_reveal){added[1].addEventListener('click',function(){if(click_reveal.style.display==='none'){click_reveal.style.display='block';}else{click_reveal.style.display='none';}
