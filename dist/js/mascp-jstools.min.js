...
big_ticks.attr({'pointer-events':'none'});little_ticks.attr({'pointer-events':'none'});big_labels.attr({'pointer-events':'none'});little_labels.attr({'pointer-events':'none'});little_ticks.attr({'stroke':'#555555','stroke-width':0.5*RS+'pt'});little_ticks.hide();little_labels.hide();huge_labels.hide();var zoom_status=null;var zoomchange=function(){if(this.zoom>3.6){if(zoom_status==1){return;}
zoom_status=1;little_ticks.hide();big_ticks.show();big_ticks.attr({'stroke-width':0.05*RS+'pt','stroke':'#999999','transform':'scale(1,0.1) translate(0,4500)'});little_labels.attr({'font-size':2*RS+'pt'});big_labels.attr({'font-size':2*RS+'pt'});axis.hide();huge_labels.hide();if(this._visibleTracers&&this._visibleTracers()){this._visibleTracers().show();}}else if(this.zoom>1.8){if(zoom_status==2){return;}
zoom_status=2;axis.show();big_ticks.show();axis.attr({'stroke-width':0.5*RS+'pt'});big_ticks.attr({'stroke-width':0.5*RS+'pt','stroke':'#000000','transform':''});big_labels.show();big_labels.attr({'font-size':4*RS+'pt','y':7*RS});little_labels.attr({'font-size':4*RS+'pt'});little_ticks.attr({'stroke-width':0.3*RS+'pt'});little_ticks.show();little_labels.show();huge_labels.hide();if(this.tracers){this.tracers.hide();}}else if(this.zoom>0.2){if(zoom_status==3){return;}
zoom_status=3;if(this.tracers){this.tracers.hide();}
axis.show();axis.attr({'stroke-width':RS+'pt'});big_ticks.show();big_ticks.attr({'stroke-width':RS+'pt','transform':'','stroke':'#000000'});big_labels.show();big_labels.attr({'font-size':7*RS+'pt','y':5*RS});little_ticks.hide();little_labels.hide();huge_labels.hide();}else{if(zoom_status==4){return;}
zoom_status=4;if(this.tracers){this.tracers.hide();}
axis.hide();big_ticks.hide();big_labels.hide();huge_labels.show();huge_labels.attr({'font-size':parseInt(30*2*RS*0.2/canvas.zoom)+'pt','y':parseInt(40*2*RS*0.2/canvas.zoom)});var last_right=-10000;huge_labels.forEach(function(label){if(label.getBBox().x<=(last_right+(RS*100))){label.setAttribute('display','none');}else{label.removeAttribute('display');last_right=label.getBBox().x+label.getBBox().width;}});little_ticks.hide();little_labels.hide();}};canvas.addEventListener('zoomChange',zoomchange,false);bean.add(axis,'removed',function(){canvas.removeEventListener('zoomChange',zoomchange);var remover=function(el){if(el.parentNode){el.parentNode.removeChild(el);}};axis.forEach(remover);big_ticks.forEach(remover)
little_ticks.forEach(remover);big_labels.forEach(remover);huge_labels.forEach(remover);little_labels.forEach(remover);});return axis;};clazz.prototype.setLeftVisibleResidue=function(val){var self=this;self._canvas.setCurrentTranslateXY((self._canvas.width.baseVal.value*(1-(val/(self.sequence.length+self.padding+2))))-self._canvas.width.baseVal.value,0);};clazz.prototype.leftVisibleResidue=function(){var self=this;var val=Math.floor((self.sequence.length+self.padding+2)*(1-((self._canvas.width.baseVal.value+self._canvas.currentTranslate.x)/self._canvas.width.baseVal.value)))-1;if(val<0){val=0;}
...
a_text.replaceChild(document.createTextNode(seq.substr(start,max_size)),a_text.firstChild);a_text.setAttribute('dx',5+((start)*RS));if(MASCP.IE){a_text.setAttribute('textLength',parseInt(5+((start)*RS))+(max_size*RS));}};var panstart=function(){if(amino_acids_shown){amino_acids.attr({'display':'none'});}};var panend=function(){if(amino_acids_shown){amino_acids.attr({'display':'block'});update_sequence();}};var zoomchange=function(){if(canvas.zoom>3.6){renderer._axis_height=14;amino_acids.attr({'display':'block'});amino_acids_shown=true;update_sequence();}else if(canvas.zoom>0.2){renderer._axis_height=30;amino_acids.attr({'display':'none'});amino_acids_shown=false;}else{renderer._axis_height=parseInt(60*2*(0.2/canvas.zoom));amino_acids.attr({'display':'none'});amino_acids_shown=false;}};if(!container.panevents){canvas.addEventListener('panstart',panstart,false);bean.add(canvas,'panend',panend);container.panevents=true;}
