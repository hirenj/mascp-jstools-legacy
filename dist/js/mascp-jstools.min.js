...
return-1*seq.length;};})();var draw_discontinuity=function(canvas){var top=-3;var left=-2;var group=canvas.group();var line;line=canvas.line(left+1,top+4,left+3,top+1);line.setAttribute('stroke','#faa');line.setAttribute('stroke-width','10');group.push(line);line=canvas.line(left+1,top+6,left+3,top+3);line.setAttribute('stroke','#faa');line.setAttribute('stroke-width','10');group.push(line);line=canvas.line(left+1,top+4,left+3,top+3);line.setAttribute('stroke','#faa');line.setAttribute('stroke-width','5');group.push(line);line=canvas.line(left+1,top+5.3,left+1,top+5.8);line.setAttribute('stroke','#faa');line.setAttribute('stroke-width','10');group.push(line);line=canvas.line(left+1,top+5.9,left+1.5,top+5.9);line.setAttribute('stroke','#faa');line.setAttribute('stroke-width','10');group.push(line);var circle=canvas.circle(left+2.8,top+1.75,1);circle.setAttribute('fill','#fff');circle.setAttribute('stroke','#aaa');circle.setAttribute('stroke-width','10');group.push(circle);var minus=canvas.text(left+2.25,top+2.25,'÷');minus.setAttribute('fill','#aaa');minus.setAttribute('font-size',100);group.push(minus);return group;};MASCP.ClustalRunner.prototype.setupSequenceRenderer=function(renderer){var self=this;var elements_to_move=[];jQuery(renderer).bind('readerRegistered',function(ev,reader){if(self==reader){return;}
...
for(var i=0;i<aligned.length;i++){var layname=self.sequences[i].agi.toLowerCase()||"missing"+i;MASCP.registerLayer(layname,{'fullname':layname.toUpperCase(),'group':group_name,'color':'#ff0000'});var text_array=renderer.renderTextTrack(layname,aligned[i].toString());rendered_bits=rendered_bits.concat(text_array);rendered_bits.slice(-1)[0].layer=layname;if(renderer.trackOrder.indexOf(layname.toLowerCase())<0){renderer.trackOrder.push(layname.toLowerCase());}
