...
do_request("www.googleapis.com","/drive/v2/files/"+item_id,null,function(err,data){if(cached_files[filename]){callback.call(null,null,cached_files[filename]);return;}
if(err){callback.call(null,err);return;}
var uri=parseUri(data.downloadUrl);do_request(uri.host,uri.relative,null,function(err,data){if(cached_files[filename]){callback.call(null,null,cached_files[filename]);return;}
if(err){callback.call(null,err);return;}
if(!data){data={};}
if(typeof data!=='string'){cached_files[filename]=data;}else{cached_files[filename]=JSON.parse(data);}
callback.call(null,null,cached_files[filename],item_id);});});});};var write_file=function(filename,mime,callback){if(!cached_files[filename]){callback.call(null,{"error":"No file to save"});return;}
...
var string_rep;try{string_rep=JSON.stringify(cached_files[filename]);}catch(e){callback.call(null,{"status":"JSON error","error":e});return;}
var headers_block={'Content-Type':mime};var req=gapi.client.request({'path':"/upload/drive/v2/files/"+item_id,'method':"PUT",'params':{"uploadType":"media"},'headers':headers_block,'body':string_rep});req.execute(function(isjson,data){if(isjson&&isjson.error&&isjson.error.code==412){cached_files[filename]=null;etags[filename]=null;get_file(filename,mime,callback);return;}
if(!isjson){callback.call(null,{"status":"Google error","response":response});return;}
etags[filename]=null;get_file(filename,mime,null);callback.call(null,null,cached_files[filename]);});});};get_permissions=function(doc,callback){var doc_id=doc.replace(/^file:/,'');get_permissions_id(function(error,permissionId){if(error){callback.call(null,error);return;}
...
return get_file(prefs_domain,"application/json; data-type=domaintool-session",callback);};MASCP.GoogledataReader.prototype.writePreferences=function(prefs_domain,callback){return write_file(prefs_domain,"application/json; data-type=domaintool-session",callback);};MASCP.GoogledataReader.prototype.getSyncableFile=function(file,callback){var file_block={"getData":function(){return"Not ready";}};get_file(file,"application/json",function(err,filedata,file_id){if(err){callback.call(null,err);}
...
timeout=setTimeout(function(){var wanting_new_sync=false;file_block.sync=function(){wanting_new_sync=true;};write_file(file,"application/json",function(err){timeout=null;file_block.sync=original_sync;if(wanting_new_sync){file_block.sync();}});},500);};original_sync=file_block.sync;if(false&&file_id){get_permissions(file_id,function(err,permissions){file.permissions=permissions;bean.fire(file_block,'ready');callback.call(null,null,file_block);});}else{file_block.permissions={"read":true,"write":true};bean.fire(file_block,'ready');callback.call(null,null,file_block);}});return file_block;};MASCP.GoogledataReader.prototype.addWatchedDocument=function(prefs_domain,doc_id,parser_function,callback){var reader=(new MASCP.GoogledataReader()).createReader(doc_id,parser_function);reader.bind('error',function(err){callback.call(null,err);});reader.bind('ready',function(){var title=this.title;(new MASCP.GoogledataReader()).getPreferences(prefs_domain,function(err,prefs){if(err){callback.call(null,{"status":"preferences","original_error":err});return;}
if(!prefs.user_datasets){prefs.user_datasets={};}
prefs.user_datasets[reader.datasetname]=prefs.user_datasets[reader.datasetname]||{};prefs.user_datasets[reader.datasetname].parser_function=parser_function.toString();prefs.user_datasets[reader.datasetname].title=title;(new MASCP.GoogledataReader()).writePreferences(prefs_domain,function(err,prefs){if(err){callback.call(null,{"status":"preferences","original_error":err});return;}
callback.call(null,null,title);});});});};MASCP.GoogledataReader.prototype.removeWatchedDocument=function(prefs_domain,doc_id,callback){this.getPreferences(prefs_domain,function(err,prefs){if(err){callback.call(null,{"status":"preferences","original_error":err});return;}
if(!prefs.user_datasets){prefs.user_datasets={};}
if(doc_id in prefs.user_datasets){delete prefs.user_datasets[doc_id];}else{callback.call();}
(new MASCP.GoogledataReader()).writePreferences(prefs_domain,function(err,prefs){if(err){callback.call(null,{"status":"preferences","original_error":err});return;}
...
var sets=prefs.user_datasets;callback.call(null,null,sets);});};MASCP.GoogledataReader.prototype.readWatchedDocuments=function(prefs_domain,callback){var gdata=new MASCP.GoogledataReader();gdata.getPreferences(prefs_domain,function(err,prefs){if(err){if(err.cause==="No user event"){console.log("Consuming no user event");return;}
