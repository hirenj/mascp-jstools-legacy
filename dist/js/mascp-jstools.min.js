...
if(initing_auth){waiting_callbacks.push(cback);}
...
setTimeout(function(){gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;window.setTimeout(function(){console.log("Google token has timed out, forcing refresh");delete MASCP["GOOGLE_AUTH_TOKEN"];},parseInt(result.expires_in)*1000);initing_auth=false;cback.call(null);waiting_callbacks.forEach(function(cb){cb.call(null);});waiting_callbacks=[];return;}else if(result&&result.error){initing_auth=false;var error={"cause":result.error};cback.call(null,error);waiting_callbacks.forEach(function(cb){cb.call(null,error);});}else{initing_auth=false;if(auth_settings.immediate){if(!user_action){cback.call(null,{"cause":"No user event","authorize":function(success){auth_settings.immediate=false;gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;window.setTimeout(function(){console.log("Google token has timed out, forcing refresh");delete MASCP["GOOGLE_AUTH_TOKEN"];},parseInt(result.expires_in)*1000);success.call(null);}else{success.call(null,{"cause":result?result.error:"No auth result"});}});}});return;}
...
prefs.user_datasets[reader.datasetname]=prefs.user_datasets[reader.datasetname]||{};prefs.user_datasets[reader.datasetname].parser_function=parser_function.toString();(new MASCP.GoogledataReader()).writePreferences(prefs_domain,function(err,prefs){if(err){callback.call(null,{"status":"preferences","original_error":err});return;}
callback.call(null,null,title);});});});};MASCP.GoogledataReader.prototype.readWatchedDocuments=function(prefs_domain,callback){var gdata=new MASCP.GoogledataReader();gdata.getPreferences(prefs_domain,function(err,prefs){if(err){if(err.cause==="No user event"){console.log("Consuming no user event");return;}
