...
if(MASCP.ready){MASCP.ready();}};};}else{if(MASCP.events){MASCP.events.emit("ready");}
if(MASCP.ready){MASCP.ready();}}};}else{try{db=openDatabase("cached","","MASCP Gator cache",1024*1024);}catch(err){throw err;}
...
if(MASCP.ready){MASCP.ready();}}});version=1.3;}});var old_get_db_data=null;begin_transaction=function(callback,trans){if(old_get_db_data!==null){callback.call({"transaction":trans});return false;}
...
a_temp_reader.retrieve(entry,function(){get_data(this.result._raw_data.etag);});});})();var trans;var get_data=function(etag){var update_timestamps={};if(typeof module=='undefined'||!module.exports&&typeof window!='undefined'){if(window.sessionStorage){update_timestamps=JSON.parse(window.sessionStorage.getItem("update_timestamps")||"{}");}}
...
reader.map=map;reader.setData(doc,data);});});}
...
extended_elements.push(canvas);canvas.makeEl=function(name,attributes){var result=document.createElementNS(svgns,name);for(var attribute in attributes){if(attributes.hasOwnProperty(attribute)){result.setAttribute(attribute,attributes[attribute]);}}
...
var transform_attr=tracer_marker.getAttribute('transform');var matches=/translate\(.*,(.*)\) scale\((.*)\)/.exec(transform_attr);if(matches[1]&&matches[2]){var scale=parseFloat(matches[2]);var y=parseFloat(matches[1]);var new_height=y+scale*(((tracer_marker.offset||0)*50)+125)-parseInt(this.getAttribute('y'));this.setAttribute('height',new_height);}else{this.setAttribute('height',height);}};if(typeof opts.offset=='undefined'||opts.offset===null){}else{tracer_marker.offset=opts.offset;}
...
var layer_containers=this._layer_containers||[];if(layer_containers[layer.name]){layer_containers[layer.name].forEach(function(el){if(el.parentNode){el.parentNode.removeChild(el);}});this.removeAnnotations(layer);this._layer_containers[layer.name]=null;layer.disabled=true;}};var refresh_id=0;clazz.prototype.enablePrintResizing=function(){if(this._media_func){return this._media_func;}
