...
el.move(aa,aa_width-aa);}});var aligned=result.getSequences();if(!renderer.sequence){renderer.setSequence(aligned[sequence_index])(function(){MASCP.registerGroup(group_name,'Splice variants');MASCP.registerLayer(controller_name,{'fullname':'Splices','color':'#000000'});if(renderer.trackOrder.indexOf(controller_name)<0){renderer.trackOrder.push(controller_name);}
...
for(var i=0;i<aligned.length;i++){var layname=self.sequences[i].agi||"missing"+i;MASCP.registerLayer(layname,{'fullname':layname,'group':group_name,'color':'#ff0000'});var text_array=renderer.renderTextTrack(layname,aligned[i].toString());rendered_bits=rendered_bits.concat(text_array);rendered_bits.slice(-1)[0].layer=layname;if(renderer.trackOrder.indexOf(layname)<0){renderer.trackOrder.push(layname);}
