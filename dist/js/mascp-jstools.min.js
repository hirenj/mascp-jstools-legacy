...
var scope="https://docs.google.com/feeds/ https://spreadsheets.google.com/feeds/";var parsedata=function(data){var entryidRC=/.*\/R(\d*)C(\d*)/;var retdata={};retdata.data=[];var max_rows=0;for(var l in data.feed.entry)
...
var doc_id=doc.replace(/^spreadsheet:/,'');get_document_using_script(doc_id,function(err,dat){if(err){authenticate();var feedUrl="https://spreadsheets.google.com/feeds/cells/"+doc_id+"/1/private/basic";var service=new google.gdata.client.GoogleService('wise','gator');var headers_block={'GData-Version':'3.0'};if(etag){headers_block["If-None-Match"]=etag;}
service.setHeaders(headers_block);service.getFeed(feedUrl,function(data){callback.call(null,null,parsedata(data));},callback,google.gdata.Feed);}else{callback.call(null,null,dat);}});};}
var render_site=function(renderer){var self=this;var sites=self.result._raw_data.sites||[],i=0,match=null;MASCP.registerLayer(self.datasetname,{'fullname':self.result._raw_data.title});for(i=sites.length-1;i>=0;i--){if(match=sites[i].match(/(\d+)/g)){renderer.getAminoAcidsByPosition([parseInt(match[0])])[0].addToLayer(self.datasetname);}}};var render_peptides=function(renderer){var self=this;var peptides=self.result._raw_data.peptides||[],i=0,match=null;MASCP.registerLayer(self.datasetname,{'fullname':self.result._raw_data.title});for(i=peptides.length-1;i>=0;i--){if(match=peptides[i].match(/(\d+)/g)){renderer.getAminoAcidsByPosition(parseInt(match[0])).addToLayer(self.datasetname);}}};var setup=function(renderer){this.bind('resultReceived',function(e){render_peptides.call(this,renderer);render_site.call(this,renderer);});};MASCP.GoogledataReader.prototype.getDocumentList=get_document_list;MASCP.GoogledataReader.prototype.getDocument=get_document;MASCP.GoogledataReader.prototype.createReader=function(doc,map){var self=this;var reader=new MASCP.UserdataReader();reader.datasetname=doc;reader.setupSequenceRenderer=setup;if(typeof google=='undefined'&&typeof document!=='undefined'){return reader;}
MASCP.Service.CacheService(reader);(function(){var a_temp_reader=new MASCP.UserdataReader();a_temp_reader.datasetname=doc;MASCP.Service.CacheService(a_temp_reader);MASCP.Service.CachedAgis(a_temp_reader,function(accs){if(accs.length<1){get_data(null);return;}
