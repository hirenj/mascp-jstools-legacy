...
var doc_id=doc.replace(/^spreadsheet:/g,'');if(!is_spreadsheet||etag||MASCP.GOOGLE_AUTH_TOKEN){basic_get_document(doc,etag,function(err,dat){if(err){get_document_using_script(doc_id,callback);}else{callback.call(null,null,dat);}});}else{get_document_using_script(doc_id,function(err,dat){if(err){basic_get_document(doc,etag,function(err,dat){if(err){if(err.cause=="No user event"){callback.call(null,err);return;}
