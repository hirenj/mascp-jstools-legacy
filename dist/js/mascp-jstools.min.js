...
marker.push(marker.contentElement);}else if(Array.isArray&&Array.isArray(symbol)){marker.contentElement=this.group();var phase=(2*Math.PI/symbol.length);phase-=(Math.PI/2);var needs_stretch=opts.stretch;symbol.forEach(function(symb,i){var new_el;var x_pos=0.5+(3*Math.cos(i*phase));var y_pos=0.5+(3*Math.sin(i*phase));var rotate_amount=360*i/symbol.length;rotate_amount-=90;if(needs_stretch){if(rotate_amount>90&&rotate_amount<270){opts.stretch='left';}else{opts.stretch='right';}}
...
var dim={CX:cx+units,CY:cy+units,R:r+units,MIN_X:(cx-r)+units,MAX_X:(cx+r)+units,MIN_Y:(cy-r)+units,MAX_Y:(cy+r)+units,MID_X1:(cx-(r/2))+units,MID_X2:(cx+(r/2))+units,MID_Y1:(cy-(r/2))+units,MID_Y2:(cy+(r/2))+units};var marker_group=this.group();var text=this.text(0,dim.CY-0.5*dim.R,txt);text.setAttribute('font-size',(opts.font_size||r)*RS);text.setAttribute('font-weight',opts.weight||'bolder');text.setAttribute('fill',opts.text_fill||'#ffffff');text.setAttribute('style','font-family: sans-serif; text-anchor: middle;');text.firstChild.setAttribute('dy','1.5ex');text.setAttribute('text-anchor','middle');var back;if(!opts.stretch){back=this.circle(0,dim.CY,9/10*dim.R);}else{var text_width=text.getBBox().width/RS;var text_height=3/2*dim.R;var left_pos=-0.5*text_width;if(text_width>(3*dim.R)){left_pos=-0.5*text_width;if(opts.stretch=='right'){left_pos=-0.1*text_width;}
