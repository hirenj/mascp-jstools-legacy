...
var old_get_aas=renderer.getAminoAcidsByPosition;var old_get_pep=renderer.getAminoAcidsByPeptide;var orig_functions={};renderer._extendElement(orig_functions);var extender=function(aas){return function(el){var result={};result.original_index=aas.shift();result.addShapeOverlay=function(layername,width,opts){elements_to_move.push(orig_functions['addShapeOverlay'].call(el,layername,(self.result.calculatePositionForSequence(index,result.original_index+width)-el._index),opts));elements_to_move.slice(-1)[0].layer_idx=index;elements_to_move.slice(-1)[0].aa=result.original_index;elements_to_move.slice(-1)[0].aa_width=width;return elements_to_move.slice(-1)[0];};result.addBoxOverlay=function(layername,width,fraction){elements_to_move.push(orig_functions['addBoxOverlay'].call(el,layername,(self.result.calculatePositionForSequence(index,result.original_index+width)-el._index),fraction));return elements_to_move.slice(-1)[0];};result.addTextOverlay=function(layername,width,opts){elements_to_move.push(orig_functions['addTextOverlay'].call(el,layername,(self.result.calculatePositionForSequence(index,result.original_index+width)-el._index),opts));return elements_to_move.slice(-1)[0];};result.addToLayerWithLink=function(layername,url,width){elements_to_move.push(orig_functions['addToLayerWithLink'].call(el,layername,url,(self.result.calculatePositionForSequence(index,result.original_index+width)-el._index)));return elements_to_move.slice(-1)[0];};for(var method in orig_functions){if(!result[method]){result[method]=function(){elements_to_move.push(orig_functions[method].apply(el,arguments));return elements_to_move.slice(-1)[0];};}}
return result;};};renderer.getAminoAcidsByPosition=function(aas){var new_aas=aas.map(function(aa){return Math.abs(self.result.calculatePositionForSequence(index,aa));});return old_get_aas.call(this,new_aas).map(extender(aas));};renderer.getAminoAcidsByPeptide=function(){};old.call(reader);renderer.getAminoAcidsByPosition=old_get_aas;renderer.getAminoAcidsByPeptide=old_get_pep;}});var rendered_bits=[];var controller_name='isoform_controller';var group_name='isoforms';var redraw_alignments=function(sequence_index){var result=self.result;while(rendered_bits.length>0){var bit=rendered_bits.shift();renderer.remove(bit.layer,bit);}
result.alignToSequence(sequence_index||0);elements_to_move.forEach(function(el){if(el.move){var aa=result.calculatePositionForSequence(el.layer_idx,el.aa);var aa_width=result.calculatePositionForSequence(el.layer_idx,el.aa_width);el.move(aa,aa_width);}});var aligned=result.getSequences();if(!renderer.sequence){renderer.setSequence(aligned[sequence_index])(function(){MASCP.registerGroup(group_name,'Splice variants');MASCP.registerLayer(controller_name,{'fullname':'Splices','color':'#000000'});if(renderer.trackOrder.indexOf(controller_name)<0){renderer.trackOrder.push(controller_name);}
...
var alignments=result.getAlignment().split('');rendered_bits.concat(renderer.renderTextTrack(controller_name,result.getAlignment().replace(/ /g,'.')));for(var i=0;i<alignments.length;i++){rendered_bits.push(renderer.getAA(i+1).addBoxOverlay(controller_name,1,check_values(aligned[0],i,aligned)));rendered_bits.slice(-1)[0].layer=controller_name;}
...
tracer_marker.setAttribute('visibility','hidden');this._renderer._layer_containers[layerName].push(tracer_marker);return[tracer,tracer_marker,bobble];};var addBoxOverlayToElement=function(layerName,width,fraction){if(typeof fraction=='undefined'){fraction=1;}
