...
var transform_attr=tracer_marker.getAttribute('transform');var matches=/translate\(.*,(.*)\) scale\((.*)\)/.exec(transform_attr);var bbox;if(renderer.sequence.length>1000){if(!canvas.cachedBBox)
{canvas.cachedBBox=[];}
if(matches[1]&&matches[2]){bbox=canvas.cachedBBox[matches[1]+"-"+matches[2]];if(!bbox){bbox=canvas.transformedBoundingBox(tracer_marker);}
canvas.cachedBBox[matches[1]+"-"+matches[2]]=bbox;}}else{bbox=canvas.transformedBoundingBox(tracer_marker);}
if(bbox&&bbox.y>0){var new_y=bbox.y-parseInt(this.getAttribute('y'));this.setAttribute('height',new_y>=0?new_y:height);}else{this.setAttribute('height',height);}};if(typeof opts.offset=='undefined'||opts.offset===null){}else{tracer_marker.offset=opts.offset;}
