...
cached_files[filename]=data;callback.call(null,null,cached_files[filename],item_id);});});};var get_file=function(file,mime,callback){gapi.auth.checkSessionState({'client_id':MASCP.GOOGLE_CLIENT_ID,'session_state':null},function(loggedOut){if(loggedOut){callback.call(null,{"cause":"No user event"});return;}
...
script.src="https://spreadsheets.google.com/feeds/cells/"+doc_id+"/1/"+type+"/basic?alt=json-in-script&callback=cback"+doc_id+""+auth;var error_function=function(e){if(window.removeEventListener){window.removeEventListener('error',error_function);}
