...
cached_files[filename]=data;callback.call(null,null,cached_files[filename],item_id);});});};var get_file=function(file,mime,callback){if(!gapi||!gapi.auth||!gapi.auth.authorize){callback.call(null,{"cause":"No google auth library"});return;}
gapi.auth.checkSessionState({'client_id':MASCP.GOOGLE_CLIENT_ID,'session_state':null},function(loggedOut){if(loggedOut){callback.call(null,{"cause":"No user event"});return;}
...
if(is_spreadsheet){get_document_using_script(doc_id,function(err,dat){if(err){get_document_using_script(doc_id,callback,true);}else{callback.call(null,err,dat);}},false);}else{callback.call(null,err);}}else{callback.call(null,null,dat);}});}else{get_document_using_script(doc_id,function(err,dat){if(err){gapi.auth.checkSessionState({'client_id':MASCP.GOOGLE_CLIENT_ID,'session_state':null},function(loggedOut){if(loggedOut==false){basic_get_document(doc,etag,function(err,dat){if(err){if(err.cause=="No user event"||err.cause=="Access is denied."){callback.call(null,err);return;}
...
callback.call(null,{"status":"preferences","original_error":err});return;}
var sets=prefs.user_datasets;for(var set in sets){(function(){var pref=sets[set];if(sets[set].type=="liveClass"){var reader_class=MASCP[set];callback.call(null,null,pref,new reader_class());return;}
if(sets[set].type=="gatorURL"){var reader=new MASCP.UserdataReader(null,set);reader.datasetname=pref.title;reader.requestData=function(){var agi=this.agi.toLowerCase();var gatorURL=set.slice(-1)=='/'?set+agi:set+'/'+agi;return{type:"GET",dataType:"json",url:gatorURL,data:{'agi':agi,'service':this.datasetname}};};callback.call(null,null,pref,reader);return;}
if(!sets[set].parser_function){return;}
var parser=eval("("+sets[set].parser_function+")");var a_reader=self.createReader(set,parser);a_reader.bind('ready',function(){callback.call(null,null,pref,a_reader);});a_reader.bind('error',function(err){callback.call(null,{"error":err});});})();}});};MASCP.GoogledataReader.prototype.createReader=function(doc,map){var self=this;var reader=new MASCP.UserdataReader(null,null);reader.datasetname=doc;reader.setupSequenceRenderer=setup;MASCP.Service.CacheService(reader);var trans;var get_data=function(etag){var update_timestamps={};if(typeof module=='undefined'||!module.exports&&typeof window!='undefined'){if(window.sessionStorage){update_timestamps=JSON.parse(window.sessionStorage.getItem("update_timestamps")||"{}");}}
...
if(typeof this.map=='function'){dataset=this.map(data);}
if(!this.map){return;}
...
zoomFunctions.push(underlayFunc);};MASCP.CondensedSequenceRenderer.prototype.renderObjects=function(track,objects){var renderer=this;objects.forEach(function(object){var click_reveal;if(object.type==="box"){if(object.aa){renderer.getAA(parseInt(object.aa)).addBoxOverlay(track,parseInt(object.width),1,object.options);}else if(object.peptide){renderer.getAminoAcidsByPeptide(object.peptide).addToLayer(track,object.options);}}
...
renderer.refresh();},false);}}});};MASCP.CondensedSequenceRenderer.prototype.addTextTrack=function(seq,container){var RS=this._RS;var renderer=this;var max_length=300;var canvas=renderer._canvas;var seq_chars=seq.split('');var amino_acids=canvas.set();var amino_acids_shown=false;var x=0;var has_textLength=true;var no_op=function(){};try{var test_el=document.createElementNS(svgns,'text');test_el.setAttribute('textLength',10);no_op(test_el.textLength);}catch(e){has_textLength=false;}
