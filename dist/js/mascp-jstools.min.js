...
callback.call(null,null,results);});};get_permissions_id=function(callback){do_request("www.googleapis.com","/drive/v2/about",null,function(err,data){callback.call(null,err,data.permissionId);});}
...
var doc_id=doc.replace(/^spreadsheet:/,'');get_permissions_id(function(err,permissionId){if(err){callback.call(null,err);return;}
do_request("www.googleapis.com","/drive/v2/files/"+doc_id+"/permissions",null,function(e,data){if(err){if(err.cause&&err.cause.status=='400'){callback.call(null,null,{"write":false,"read":false});return;}
...
head.appendChild(script);};authenticate=function(cback){if(!gapi||!gapi.auth.authorize){cback.call(null,{"cause":"No google auth library"});return;}
if(!MASCP.GOOGLE_CLIENT_ID){cback.call(null,{"cause":"No client ID set (MASCP.GOOGLE_CLIENT_ID)"});return;}
var auth_settings={client_id:MASCP.GOOGLE_CLIENT_ID,scope:scope,immediate:true};gapi.auth.authorize(auth_settings,function(result){if(result&&!result.error){MASCP.GOOGLE_AUTH_TOKEN=result.access_token;cback();}else{if(auth_settings.immediate){auth_settings.immediate=false;gapi.auth.authorize(auth_settings,arguments.callee);}}});return;};do_request=function(host,path,etag,callback){authenticate(function(err){if(err){callback.call(null,err);return;}
