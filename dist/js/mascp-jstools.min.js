...
if(JSandbox){var sandbox=new JSandbox();var parser;sandbox.exec('var sandboxed_parser = '+pref.parser_function+';',function(){var box=this;parser=function(datablock,cback){box.eval({"data":"sandboxed_parser(datablock)","input":{"datablock":datablock},"callback":function(r){cback.call(null,r);console.log("Terminating");box.terminate();}});sandbox.terminate();};parser.callback=true;parser.terminate=function(){if(sandbox){sandbox.terminate();}};var a_reader=(new MASCP.GoogledataReader()).createReader(set,parser);a_reader.bind('ready',function(){if(parser){parser.terminate();}
...
if(typeof this.map=='function'){if(this.map.callback){var self_func=arguments.callee;this.map(data,function(parsed){self.map=function(d){return(d);};self.call(arguments.callee,name,parsed);});return;}
