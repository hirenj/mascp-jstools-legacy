...
if(MASCP.ready){MASCP.ready();}else{MASCP.ready=true;}
return;}
...
if(MASCP.ready){MASCP.ready();}else{MASCP.ready=true;}}});version=1.3;}});var old_get_db_data=null;begin_transaction=function(callback,trans){if(old_get_db_data!==null){callback.call({"transaction":trans});return false;}
...
setTimeout(function(){if(MASCP.ready){MASCP.ready();}else{MASCP.ready=true;}},100);}else{sweep_cache=function(timestamp){};clear_service=function(service,acc){};search_service=function(service,cback){};cached_accessions=function(service,cback){cback.call(clazz,[]);};get_db_data=function(acc,service,cback){cback.call(null,null,null);};store_db_data=function(acc,service,data){};find_latest_data=function(acc,service,timestamp,cback){cback.call(null,[]);};data_timestamps=function(service,timestamp,cback){cback.call(null,[]);};begin_transaction=function(callback){setTimeout(function(){callback();},0);};end_transaction=function(callback){setTimeout(function(){callback();},0);};}})(MASCP.Service);MASCP.Service.prototype.setAsync=function(asyncFlag)
...
var re=/^#cdsid\t([a-zA-Z0-9-]+)/m;var match;if(typeof data=="string"&&!this.job_id){match=re.exec(data);if(match){this.job_id=match[1];this.retrieve(this.agi);return;}}
re=/^#status\tsuccess/m;if(re.exec(data)){this.job_id=null;return defaultDataReceived.call(this,data,status);}
re=/#status\t3/m;if(re.exec(data)){var self=this;setTimeout(function(){self.retrieve(self.agi);},1000);return;}
...
function parseUri(str){var o=parseUri.options,m=o.parser[o.strictMode?"strict":"loose"].exec(str),uri={},i=14;while(i--)uri[o.key[i]]=m[i]||"";uri[o.q.name]={};uri[o.key[12]].replace(o.q.parser,function($0,$1,$2){if($1)uri[o.q.name][$1]=$2;});return uri;};parseUri.options={strictMode:false,key:["source","protocol","authority","userInfo","user","password","host","port","relative","path","directory","file","query","anchor"],q:{name:"queryKey",parser:/(?:^|&)([^&=]*)=?([^&]*)/g},parser:{strict:/^(?:([^:\/?#]+):)?(?:\/\/((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?))?((((?:[^?#\/]*\/)*)([^?#]*))(?:\?([^#]*))?(?:#(.*))?)/,loose:/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/}};get_preferences=function(prefs_domain,callback){if(!MASCP.preferences){MASCP.preferences={};}
if(!prefs_domain){prefs_domain="MASCP GATOR PREFS";}
if(MASCP.preferences[prefs_domain]){callback.call(null,null,MASCP.preferences[prefs_domain]);return;}
var query=encodeURIComponent("title='"+prefs_domain+"' and 'appdata' in parents and mimeType = 'application/json; data-type=domaintool-session' and trashed = false");do_request("www.googleapis.com","/drive/v2/files?q="+query,null,function(err,data){if(err){callback.call(null,err);return;}
if(data.items.length==0){MASCP.preferences[prefs_domain]={};callback.call(null,null,MASCP.preferences[prefs_domain]);return;}
...
if(typeof data!=='string'){MASCP.preferences[prefs_domain]=data;}else{MASCP.preferences[prefs_domain]=JSON.parse(data);}
callback.call(null,null,MASCP.preferences[prefs_domain]);});});});};write_preferences=function(prefs_domain,callback){if(!MASCP.preferences){MASCP.preferences={};}
if(!prefs_domain){prefs_domain="Domaintool preferences";}
if(!MASCP.preferences[prefs_domain]){callback.call(null,{"error":"No preferences to save"});return;}
var query=encodeURIComponent("title='"+prefs_domain+"' and 'appdata' in parents and mimeType = 'application/json; data-type=domaintool-session' and trashed = false");do_request("www.googleapis.com","/drive/v2/files?q="+query,null,function(err,data){if(err){callback.call(null,err);return;}
var item_id=null;if(data.items&&data.items.length==0){do_request("www.googleapis.com","/drive/v2/files/",null,arguments.callee,"POST:application/json",JSON.stringify({'parents':[{'id':'appdata'}],"title":prefs_domain,"mimeType":"application/json; data-type=domaintool-session","description":"Domaintool session information for session "+prefs_domain}));return;}
if(data.items){item_id=data.items[0].id;}else{item_id=data.id;}
var req=gapi.client.request({'path':"/upload/drive/v2/files/"+item_id,'method':"PUT",'params':{"uploadType":"media"},'headers':{'Content-Type':'application/json; data-type=domaintool-session'},'body':JSON.stringify(MASCP.preferences[prefs_domain])});req.execute(function(isjson,data){if(!isjson){callback.call(null,{"status":"Google error","response":response});return;}
callback.call(null,null,MASCP.preferences[prefs_domain]);});});};get_permissions=function(doc,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');get_permissions_id(function(error,permissionId){if(error){callback.call(null,error);return;}
